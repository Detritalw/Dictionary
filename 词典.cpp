...
void error(int a){
	...
	errorfind=scanf("%d",&errors);
	if(errorfind<1) error(99);
...
void all_add(){
	new_k();
	system("title 读入库数据...");
	if(scanf("%d%d",&x_num,&jm)==-1) error(101);
	else{
		for(int i=1;i<=x_num;i++){
			cin>>k[i][0]>>k[i][1];
			if(!jm) cout<<"读入项目"<<i<<"的数据...("<<i<<"/"<<x_num<<") name:"<<k[i][0]<<" 简介:"<<k[i][1]<<"\n";
		}
	}
...
void find(){
	MessageBox(NULL,"请查看控制台,若此窗口无响应是正常情况", "查询内容", MB_OK | MB_ICONINFORMATION);
	system("cls");
	system("title 词典 - 查询");
	...
	for(int v=1;v<=x_num;v++){
		for(int i=1;i<=k_num;i++){
			if(!jm) cout<<"正在查找"<<i<<"/"<<k_num<<" 内容:"<<k[i][0]<<"\n";
			if(k[i][1]==f){
				finres[findres][0]=i;
				finres[findres][1]=v;
				findres++;
...
void cdnew(int xh){
	system("cls");
	system("title 警告");
	cout<<"您当前的操作有可能会导致词典数据丢失\n\n是否确认更换(Y/y/1/q/Q 确认)";res=getch();
	switch(res[0]){
		case 'Y':break;
		case 'y':break;
		case '1':break;
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow){
	system("title 词典");
	boxreturn=MessageBox(NULL,"词典由碎屑制作\n作者对于C++语言Windows窗口GUI图形界面的技术不太好,部分功能可能需要您使用控制台(即后方的黑色窗口)\n按下按钮后可能经常会无响应,属于正常现象\n非常感谢您的理解和使用\n联系方式:Detritalw@outlook.com\n\n您要打开默认库data.cd吗?\n按下取消将询问词典库名称","词典",MB_SYSTEMMODAL|MB_OKCANCEL|MB_ICONQUESTION);
	if(boxreturn==2){
		cout<<"请输入您的词典库文件名称(带后缀名)";
		cin>>firname;
		freopen(firname.data(),"r",stdin);
	}else freopen("data.cd","r",stdin);
	all_add();
	WNDCLASSEX wc;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch(uMsg){
		case WM_CREATE:{
			...
			CreateWindow("BUTTON","查询",WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON,      50,100,100,250,hwnd,(HMENU)1,NULL,NULL);
			CreateWindow("BUTTON","zyk",WS_VISIBLE|WS_CHILD |BS_DEFPUSHBUTTON,      150,100,100,250,hwnd,(HMENU)2,NULL,NULL);
			CreateWindow("BUTTON","重新获取库",WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON,250,100,100,250,hwnd,(HMENU)3,NULL,NULL);
			CreateWindow("BUTTON","输出调试库",WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON,350,100,100,250,hwnd,(HMENU)4,NULL,NULL);
			CreateWindow("BUTTON","清空词典",WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON,  450,100,100,250,hwnd,(HMENU)5,NULL,NULL);
			...
			CreateWindow("BUTTON","你好",WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON,650,100,100,100,hwnd,(HMENU)8,NULL,NULL);
			/*
...
*************************************************** ************************* ************************* ************************* ************************* ************************* ************************* ************************* ************************* 
		...
		case WM_COMMAND:{
			switch(LOWORD(wParam)){
				case 1:find();break;
				case 2:zyk();break;
				case 3:all_add();break;
				case 4:Save_123();break;
//				case 5:{
//					boxreturn=MessageBox(NULL,"确认重读词典吗?\n重读词典也会先清空词典","重读词典",MB_OKCANCEL|MB_ICONQUESTION);
//					if(boxreturn==1) new_k();
//					else boxreturn=MessageBox(NULL,"已取消","重读词典",MB_OK|MB_ICONASTERISK);
//					break;
//				}
				case 5:{
					boxreturn=MessageBox(NULL,"确认清空词典吗?","清空词典",MB_SYSTEMMODAL|MB_OKCANCEL|MB_ICONQUESTION);
					if(boxreturn==1) new_k();
					else boxreturn=MessageBox(NULL,"已取消","清空词典",MB_SYSTEMMODAL|MB_OK|MB_ICONASTERISK);
					break;
				}
				case 6:system("explorer Readme.md");break;
				case 7:system("explorer \"http://img.ess.ink:16055/词典/库下载站\"");break;
				case 8:error(103);break;
